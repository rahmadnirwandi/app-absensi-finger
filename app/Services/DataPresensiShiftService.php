...
class DataPresensiShiftService extends BaseService
{
        ...
        $query=DB::table(DB::raw(
            '(
                select
                    ...
                    nm_ruangan
                from (
                    select * from ref_karyawan_jadwal_shift where id_template_jadwal_shift = 1
                ) utama
    ...
    public function getDataRumus3($params=[]){
        ...
        $paramater_data_karyawan_shift=[
            'search'=>$form_filter_text
        ];

        if(!empty($filter_id_jabatan)){
            $paramater_data_karyawan_shift['id_jabatan']=$filter_id_jabatan;
        }

        if(!empty($filter_id_departemen)){
            $paramater_data_karyawan_shift['id_departemen']=$filter_id_departemen;
        }

        if(!empty($filter_id_ruangan)){
            $paramater_data_karyawan_shift['id_ruangan']=$filter_id_ruangan;
        }

        $list_data_karyawan_shift = $this->getListKaryawan($paramater_data_karyawan_shift, 2)->select(DB::raw('group_concat(id_karyawan) as id_karyawan'),DB::raw('group_concat(id_user) as id_user'))->first();
    
        $data_jadwal=(new \App\Http\Traits\PresensiHitungShiftFunction)->getWaktuKerja(['id_jenis_jadwal'=>2])->first();
        
        $parameter_search=[
            'search_parameter'=>[
                'tanggal'=>[$filter_date_start,$filter_date_end],
                'list_id_karyawan'=>!empty($list_data_karyawan_shift->id_karyawan) ? $list_data_karyawan_shift->id_karyawan : 0,
                'list_id_user'=>!empty($list_data_karyawan_shift->id_user) ? $list_data_karyawan_shift->id_user : '',
            ],
            'search_karyawan'=>$paramater_data_karyawan_shift,
            'limit'=>!empty($params['limit']) ? $params['limit'] : [],
        ];
        ...
        if($list_data){
            foreach($list_data as $key => $value){
                ...
                $hasil_proses=(new \App\Http\Traits\PresensiHitungshiftFunction)->getProses($data_proses);
