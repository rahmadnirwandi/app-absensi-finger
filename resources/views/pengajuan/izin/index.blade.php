...
@section('content')
@include('pengajuan.izin.tab_pengajuan_izin', ["active"=>1])
@if( (new \App\Http\Traits\AuthFunction)->checkAkses('izin/create') )
    <div class="collapse mb-2" id="bagan-form-tambah-collapse">
        <div class="card card-body" style='background:#f2f2f2'>
            <?php 
                
                $bagan_form=\App::call($router_name->base_controller.'@actionFormCreate');
            ?>
            @if(!empty($bagan_form))
                {!! $bagan_form !!}
            @endif
...
@include('pengajuan.izin.columns')
@endsection
