            ...
            <table class="table table-bordered table-hover align-middle mb-0" style="width:100%; border: 1px solid #dee2e6;">
                <thead class="table" style="background-color: #CEE5FF;">
                    <tr class="text-center">
                        ...
                        <th style="min-width: 150px">Permohonan</th>
                        <th style="min-width: 120px">Status</th>
                        <th style="min-width: 100px">File</th>
                        <th style="min-width: 200px">Disetujui Oleh</th>
                        <th style="min-width: 200px">SDI</th>
                    </tr>
                </thead>
                <tbody>
                    @if(!empty($list_data) && count($list_data) > 0)
                        @foreach($list_data as $item)
                            ...
                            @endphp
                        <tr>
                            <td class="text-center font-monospace">{{ $item->nip_pemohon ?? '-' }}</td>
                            ...
                            <td class="text-start">
                                ...
                                <div class="permohonan">
                                    @if($item->status_pengajuan === 'rejected' && $item->current_level == 1)
                                        <i class="fa-solid fa-circle-xmark text-danger"></i>
                                    @elseif($item->current_level >= 2)
                                        <i class="fa-solid fa-circle-check text-success"></i>
                                    @elseif($item->current_level == 1 && $item->status_pengajuan === 'pending')
                                ...
                                <div class="permohonan">
                                    @if($item->status_pengajuan === 'approved')
                                        <i class="fa-solid fa-circle-check text-success"></i>
                                    @elseif($item->status_pengajuan === 'rejected' && $item->current_level == 2)
                                        <i class="fa-solid fa-circle-xmark text-danger"></i>
                                    @elseif($item->current_level >= 2 && $item->status_pengajuan === 'pending')
                                        <i class="fa-regular fa-clock text-warning"></i>
                                    @else
                                        <i class="fa-regular fa-circle text-muted"></i>
                                    @endif
                            ...
                            <td class="text-center">
                                @if($item->status_approved_histories === 'approved')
                                    <span class="badge bg-success rounded-pill">Approved</span>
                                @elseif($item->status_approved_histories === 'rejected')
                            ...
                            <!-- Kolom Disetujui Oleh → selalu Level 1 (Kepala Unit) -->
                            <td class="fw-bold">
                                @if($item->nama_approver_level1)
                            ...
                            <!-- Kolom SDI → selalu Level 2 -->
                            <td class="fw-bold">
                                @if($item->nama_approver_level2)
