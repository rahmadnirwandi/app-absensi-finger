...
<div class="card border-2 mt-2">
    <div class="card-header bg-white py-3">
        <h6 class="m-0 font-weight-bold text-primary">Daftar Data Cuti Karyawan</h6>
    </div>
    <div class="card-body">
            ...
            <table class="table table-bordered table-hover align-middle mb-0" style="width:100%; border: 1px solid #dee2e6;">
                <thead class="table" style="background-color: #CEE5FF;">
                    <tr class="text-center">
                        ...
                        <th style="width: 10%">Awal Cuti</th>
                        <th style="width: 10%">Akhir Cuti</th>
                        <th style="width: 5%">Jumlah</th>
                        <th style="width: 5%">Pakai</th>
                        <th style="width: 5%">Sisa</th>
                        <th style="width: 5%">Tukar</th>
                ...
                <tbody>
                    @if(!empty($list_data) && count($list_data) > 0)
                        ...
                        <tr>
                            ...
                            <td class="text-nowrap text-center">
                                {{ $item->nama_jenis_cuti ?? '0' }}
                                
                            </td>

                            <td class="text-center">
                                <div class="btn-group gap-2" role="group">
                                    <a class="btn btn-sm btn-info modal-remote" style='color:#fff;' href="{{ $url_update }}"  data-modal-key='{{json_encode($data_modal_key)}}' data-modal-width='30%' data-modal-title='Edit Stok Cuti Karyawan'>
                                         <i class="fa-solid fa-pen-to-square"></i> Update
                                    </a>
                                </div>
                                <div class="btn-group gap-2" role="group">
                                    
                                    <a class="btn btn-sm btn-danger modal-remote-delete" style='color:#fff;' href="{{ $url_delete }}"  data-modal-key={{ $item->id }} data-modal-width='30%' data-modal-title='Informasi' data-confirm-message="Hapus stok cuti <strong>{{ $item->nm_karyawan }}</strong> ?">
        ...
        <div class="mt-3 d-flex justify-content-between align-items-center">
            <span class="small text-muted">
                Showing {{ $list_data->firstItem() }} to {{ $list_data->lastItem() }} of {{ $list_data->total() }} entries
            </span>
